...
import { buildSummary, monthRange } from '../reports/summary.js';
import { buildTrend, monthsEndingAt } from '../reports/trend.js';
...
export const monthlySummary: AuthedHandler = async (request) => {
  ...
  const totals = await request.repos.expenses.totalsByCategory(from, to);
...
export const monthlyTrend: AuthedHandler = async (request) => {
  ...
  const totals = await request.repos.expenses.totalsByMonth(`${first}-01`, `${last}-31`);
